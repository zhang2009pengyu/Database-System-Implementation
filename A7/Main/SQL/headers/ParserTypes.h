...
public:
	...
	vector <ExprTreePtr> &getAllDisjuncts () {
		return allDisjunctions;
	}
	vector <ExprTreePtr> &getAllAtts(){
		return valuesToSelect;
	}
	vector <ExprTreePtr> &getAllGroupings(){
	...
	void check (MyDB_CatalogPtr catalog) {
		...
		for (auto groupingClause : groupingClauses) {
			if (!groupingClause->ini(catalog, tablesToProcess,
									   groupingClauses)) {
				return;
			}
		...
		for (auto value : valuesToSelect) {
			if (!value->ini(catalog, tablesToProcess,
							  groupingClauses)) {
				return;
			}
		}
		
		for (auto disjunc : allDisjunctions) {
			if (!disjunc->ini(catalog, tablesToProcess,
								groupingClauses)) {
				return;
			}
		}
...
public:
	...
	void iniCheck (MyDB_CatalogPtr catalog) {
		myQuery.check(catalog);
	}

	struct SFWQuery &getSFWQuery () {
		return myQuery;
	}
	#include "FriendDecls.h"
};

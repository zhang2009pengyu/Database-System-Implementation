			...
			if (pos != string :: npos) {
				...
				// if we did parse correctly, just print out the query
					...
					if (final->isCreateTable ()) {
						...
						final->iniCheck(myCatalog);

						// construct plan
						Integrater integrater (allTableReaderWriters,final->getSFWQuery());

						MyDB_TableReaderWriterPtr output =integrater.integrate ();

						
						output->writeIntoTextFile ("testCASE.output");
						MyDB_RecordPtr temp = output->getEmptyRecord ();
						MyDB_RecordIteratorAltPtr myIter =output->getIteratorAlt ();
						int count = 0;
						cout << "The fist 30 records: " << endl;

...
void ScanJoin :: run () {
	...
	for (auto &p : leftTable->getTable ()->getSchema ()->getAtts ()){
		//cout << "left att: " << p.first << " " << p.second->toString () << endl; 
		mySchemaOut->appendAtt (p);
	}
	for (auto &p : rightTable->getTable ()->getSchema ()->getAtts ()) {
		//cout << "right att: " << p.first << " " << p.second->toString () << endl;
		mySchemaOut->appendAtt (p);
	}
	...
	//cout << "finalPredicate: " << finalSelectionPredicate << endl;

...
//       test 4 on BPlusSelection. There is only test case on BPlusSelection
		vector <string> projections;
		...
		low->set ("aa");
		high->set ("ab");
//        [r_address] > string[aa] and [r_address] < string[ab]
		BPlusSelection myOp (supplierTableR, supplierTableOut, low, high,
//                              > ([r_address], string[aa])
//                                                              => && ( > ([r_address], string[aa]), < ([r_address], string[ab]))
//                              < ([r_address], string[ab])

//                             > ([r_name], string[Supplier#000009000])
...
//   use the following two test case to test aggregate
	{
//        test 5 on Aggregate specially
		vector <pair <MyDB_AggType, string>> aggsToCompute;
...
//        test 6 on aggregate specially
...

	{
		...
		projections.push_back ("[l_nationkey]");
//      5.1 join (sort merge join . scan join)
		cout << "Do you want to run a:\n";
		cout << "\t1. Sort merge join.\n";
		cout << "\t2. Scan join.\n";
		cout << "Enter 1 or 2:\n";

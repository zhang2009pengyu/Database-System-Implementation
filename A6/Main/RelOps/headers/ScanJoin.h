...
public:
	...
	//
        ...
        //         "&& ( == ([l_suppkey], [r_suppkey]), == ([l_name], [r_name]))",
	// 				projections,
	// 				hashAtts,
	...
	ScanJoin (MyDB_TableReaderWriterPtr leftInput, MyDB_TableReaderWriterPtr rightInput,
		MyDB_TableReaderWriterPtr output,
			  string finalSelectionPredicate,  //"&& ( == ([l_suppkey], [r_suppkey]), == ([l_name], [r_name]))"
//
			  vector <string> projections, //the attributes in select
			  vector <pair <string, string>> equalityChecks,//pred use both table1 and table2
			  string leftSelectionPredicate, //"|| ( == ([l_nationkey], int[3]), == ([l_nationkey], int[4]))"
			  string rightSelectionPredicate);//"== ([r_nationkey], int[3])")

...
public:
    ...
    void check (MyDB_CatalogPtr catalog){
        ...
        catalog->getStringList("tables", inputTables);

		//check whether the table exist in the database
        for (auto inputTable : tablesToProcess) {

            bool inCatalog = false;
            for (auto table : inputTables) {
                if (inputTable.first == table) {
                    inCatalog = true;
                }
            }
            if (!inCatalog) {
                cout << "Error: The input table " << inputTable.first << " not in the database." << endl;
                return;
            }
        }
        ...
        for (auto groupingClause : groupingClauses) {
            if (!groupingClause->check(catalog, tablesToProcess, groupingClauses, false)) {
                return;
            }
        }

		//check value to select
        bool hasAgg = false;
        ...
        for (auto value : valuesToSelect) {
            if (!value->check(catalog, tablesToProcess, groupingClauses, hasAgg)) {
                return;
            }
        }

		//check disjunc
        for (auto disjoin : allDisjunctions) {
            if (!disjoin->check(catalog, tablesToProcess, groupingClauses, false)) {
                return;
            }
        }
...
public:
    ...
    void sqlCheck(MyDB_CatalogPtr catalog){
